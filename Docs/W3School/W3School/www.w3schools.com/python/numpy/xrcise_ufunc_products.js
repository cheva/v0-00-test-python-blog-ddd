qobjects = [
  ...
  {
    question: "Consider the following code:<br><code class='codeblock'>import numpy as np<br>arr1 = np.array([1, 2, 3])<br>arr2 = np.array([4, 2, 1])<br>newarr = np.prod([arr1, arr2], axis=1)<br>newarr = np.sum([arr1, arr2], axis=1)</code><br>What will be the result of <code class='w3-codespan'>newarr</code>?",
    options: [
      "<code>48</code>",      
      "<code>[6 8]</code>",      
      "<code>[4 4 3]</code>"
